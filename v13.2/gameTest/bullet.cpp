...
Bullet::Bullet(QGraphicsItem *parent, int angle, int bulletStep, int bulletShooter): QObject(), QGraphicsItem(parent)
{
    ...
    if(angle == 0 || angle == 140 || angle == 560 || angle == 280 || angle == 420 || angle == 700)//if the direction the bullet is being shooted is horizontal, rotate the img 90o
    {
...
void Bullet::move()
// if bullet shooted by enemy collides with player destoy bullet and reduce player's health
// if bullet  shooted by playe collides with enemy, destroy both
// if it collides with a bullet shooted by an enemy, both are destroyed
{
