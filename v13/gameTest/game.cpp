...
Game::Game(QWidget *parent)
{
    ...
    launchSU();
    scene->addItem(player1->segD);
    scene->addItem(player1->segR);
    scene->addItem(player1->segU);
    scene->addItem(player1->segL);
...
void Game::setPlayerHpBar()
{
    ...
    if(player1->info->hpBar->lenght <= 0)
    {
        ...
        disconnect(setfocusTimer, &QTimer::timeout, this, &Game::setFocus);
        disconnect(setplayerHpTimer, &QTimer::timeout, this, &Game::setPlayerHpBar);
...
void Game::shutdown(int gameover)
{
    if(gameover==1)//end this game
    {
        ...
        qInfo() << "shutdown emitted gameover =" , gameover;
    }
}
...
void Game::close()
{
    ...
    emit Exit(1);
    qInfo() << "close emitted exit signal";
